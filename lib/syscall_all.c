...
int sys_write_dev(int sysno, u_int va, u_int dev, u_int len)
{
        // Your code here
		u_int from = dev;
		u_int endaddr = len + dev;
		if (!(from >= 0x10000000 && endaddr <= 0x10000020)) {
			if (!(from >= 0x13000000 && endaddr <= 0x13004200)) {
				if (!(from >= 0x15000000 && endaddr <= 0x15000200)) {
					return -E_INVAL;
				}
			}
		}
		...
		u_int dstva = dev + 0xA0000000;
		bcopy((void*)va, (void*)dstva, len);
		return 0;
}	
...
int sys_read_dev(int sysno, u_int va, u_int dev, u_int len)
{
		...
		u_int srcva = dev + 0xA0000000;
		bcopy((void*)srcva, (void*)va, len);
		return 0;
}
