...
duppage(u_int envid, u_int pn)
{
	...
	if ((perm & PTE_R) == 0) {
		if (syscall_mem_map(0 ,addr, envid, addr, perm) < 0) {
			user_panic("user panic mem map error when PTE_R");
		}
	} else if (perm & PTE_LIBRARY) {
		if (syscall_mem_map(0 ,addr, envid, addr, perm) < 0) {
			user_panic("user panic mem map error when PTE_LIBRARY");
		}
	} else if (perm & PTE_COW) {
		if (syscall_mem_map(0 ,addr, envid, addr, perm) < 0) {
			user_panic("user panic mem map error when PTR_COW");
		}
	} else {
		if (syscall_mem_map(0 ,addr, envid, addr, perm | PTE_COW) < 0) {
			user_panic("user panic mem map error when trans");
		}
		if (syscall_mem_map(0 ,addr, 0, addr, perm | PTE_COW) < 0) {
			user_panic("user panic mem map error when self");
		}
